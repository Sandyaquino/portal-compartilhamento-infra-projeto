...
def _garantir_tabelas(cursor) -> None:
    ...
    # Área de atuação (EPS x MUNICIPIO): semeia se vazia.
    cursor.execute(f"SELECT COUNT(*) FROM {TB_EPS_ATUACAO}")
    if int(cursor.fetchone()[0] or 0) == 0:
        _seed_eps_atuacao(cursor)


# EPS cadastradas no portal.
EPS_SEED = ["CADIC", "ELEKTRA", "ORC", "DINAMO"]


def _seed_eps_equipes(cursor) -> None:
    ids_eps: List[int] = []
    for nome in EPS_SEED:
        cursor.execute(
            f"INSERT INTO {TB_EPS} (NOME, TIPO_SERVICO, ATIVO, CREATED_AT) VALUES (?, 'AMBOS', 'S', CURRENT_UTCTIMESTAMP)",
            [nome],
        )
    ...
    try:
        cursor.execute(
            ...
            GROUP BY MUNICIPIO ORDER BY COUNT(*) DESC LIMIT 4
            """
        )
...
def _seed_eps_atuacao(cursor) -> None:
    ...
    try:
        cursor.execute(
            ...
            GROUP BY MUNICIPIO ORDER BY COUNT(*) DESC LIMIT 12
            """
        )
        municipios = [r[0] for r in cursor.fetchall()]
    except Exception:
        municipios = []
    # cada município é atendido por 2 EPS (round-robin sobre as EPS ativas)
    for idx, mun in enumerate(municipios):
        alvo = {ids_eps[idx % len(ids_eps)]}
        if len(ids_eps) > 1:
            alvo.add(ids_eps[(idx + 1) % len(ids_eps)])
        for id_eps in alvo:
            cursor.execute(
                f"""INSERT INTO {TB_EPS_ATUACAO} (ID_EPS, MUNICIPIO, ATIVO, CREATED_AT, CREATED_BY)
...
def listar_eps_atuacao(
    id_eps: Optional[int] = Query(None),
    ...
    try:
        ...
        cursor.execute(
            f"""
            SELECT A.ID_EPS, E.NOME, A.MUNICIPIO
            FROM {TB_EPS_ATUACAO} A JOIN {TB_EPS} E ON E.ID_EPS = A.ID_EPS
            {where}
            ORDER BY E.NOME, A.MUNICIPIO
            """,
            params,
        )
        return _rows(cursor)
